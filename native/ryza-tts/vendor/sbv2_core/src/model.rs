...
/// Inference hardware device; switching device means rebuilding the session.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum InferenceDevice {
    /// CPU inference (default)
    #[default]
    Cpu,
    /// GPU acceleration (DirectML on Windows, WebGPU on Linux)
    Gpu,
    /// NPU acceleration via DirectML (Windows)
    Npu,
    /// Specific device by id (DXGI / Vulkan device index)
    Specific(i32),
}
...
/// Load a model with the given inference device; falls back to CPU if the EP is unavailable.
#[allow(clippy::vec_init_then_push, unused_variables)]
pub fn load_model_with_device<P: AsRef<[u8]>>(
    ...
    #[cfg(all(feature = "webgpu", target_os = "linux", target_arch = "x86_64"))]
    {
        // WebGPU EP: x86_64 Linux only; same device mapping as DirectML.
