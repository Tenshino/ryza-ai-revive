...
统一重出 **1.2.7**（`output/desktop/RyzaChat-Setup-1.2.7.exe`、
`output/android/RyzaChat-1.2.7.apk`，两包出厂前都过了隐私闸门；
...
| 包装 | 可安装的桌面/安卓 | **exe 安装包与 APK 均已产出（1.2.7）**，见 §5 条 5 |
...
5. 安装包：`scripts/build_apk.ps1` 需要装了便携 JDK+SDK 的机器（`setup_android_tools.ps1`
   一次性装到 D:\agent\tools）。**当前产物已出（1.2.7，版本单一来源 `config/version.json`）**：
   - `output/desktop/RyzaChat-Setup-1.2.7.exe`（617MB，NSIS 走「应用和功能」正常安装/卸载；
     `deleteAppDataOnUninstall:false` ⇒ 存档留在 %AppData%\RyzaChat，要彻底清就用设置页
     「抹除全部本地数据」；win-unpacked 自检截图已核）
   - `output/android/RyzaChat-1.2.7.apk`（564MB，自签，正常安装/卸载；新增
     `android:hasFragileUserData` ⇒ API29+ 卸载时询问是否保留数据）
   - 两包都由 `scripts/privacy_check.py` 在**暂存前 + 成品**各扫一遍（私人标识、密钥形状、
     providers.json、keystore 命中即构建失败）；1.2.7 出厂扫描零命中。
