...
* **塔奥家空档 = 暖色清屏色，不是 DOM 覆盖层**：全包 dump 证实该场景只有两块 quad
  （far_bg 墙 629..2701 + floor 木地 −1064..−2701），中间 1693u 是画师留空的房间中景纵深，
  **没有沙发**（坐姿角色 atlas 也只有人没有家具；官方白 T 坐姿截图是另一套皮肤，本包只有
  预览图无骨骼）。曾用 `#stage-gap` DOM 层填这条带子——**错**：DOM 压在 WebGL 画布之上，
  把画布里角色的下半身也盖住了。现改为把 GL 清屏色调成暖棕 (0.16,0.11,0.07)：清屏色永远在
  角色/场景背后，不可能遮她；而正常场景 far_bg 铺满窗口根本露不到清屏色，只有塔奥家这道
  空档会显出来，正好读作暗色地板。**角色前景 + 不露黑，两者同时成立。**
